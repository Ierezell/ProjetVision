...
while(1):
    ...
    try:
        # an error comes if it does not find anything in window as it cannot
        # find contour of max area therefore this try error statement
        ret, frame = cap.read()
        frame = cv2.flip(frame, 1)
        kernel = np.ones((3, 3), np.uint8)

        # define regioon of interest which is a small square on screen
